...
@AllArgsConstructor
public abstract class ReactFriendlyPieceThreatened extends ConversationAction {
    ...
    private ChessPiece getThreatenedPiece(GameState gameState) {
        var myColour = pieceAgent.getPieceContext().getMyPiece().getColour();
        return randomChooser.chooseRandom(gameState.getThreatenedForColour(myColour));
    }
}
