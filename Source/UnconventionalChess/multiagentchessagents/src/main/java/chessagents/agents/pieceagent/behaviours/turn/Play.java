...
public class Play extends PieceFSMBehaviour {
    ...
    public Play(PieceAgent pieceAgent, PieceContext pieceContext) {
        ...
        registerTransition(TELL_PIECE_TO_MAKE_MOVE, WAIT_FOR_PIECE_RESPONSE_TO_MAKE_MOVE_REQUEST, TOLD_PIECE_TO_MAKE_MOVE);

        registerTransition(WAIT_FOR_PIECE_RESPONSE_TO_MAKE_MOVE_REQUEST, WAIT_FOR_MOVE_CONFIRMATION, PIECE_AGREED_TO_MAKE_MOVE);

        registerTransition(HANDLE_ASKED_TO_MOVE, DECIDE_IF_ACTUALLY_MOVING, AGREED_TO_MAKE_MOVE);

        registerTransition(DECIDE_IF_ACTUALLY_MOVING, REQUEST_GAME_AGENT_MOVE_AND_AWAIT_CONFIRMATION, ACTUALLY_MAKING_MOVE);

        registerTransition(REQUEST_GAME_AGENT_MOVE_AND_AWAIT_CONFIRMATION, PERFORM_MOVE, RECEIVED_CONFIRMATION_OF_MOVE_MADE);

        registerTransition(WAIT_FOR_MOVE_CONFIRMATION, PERFORM_MOVE, RECEIVED_CONFIRMATION_OF_MOVE_MADE);
    }
}
