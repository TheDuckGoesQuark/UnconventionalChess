...
Conversation messages contain the name of the piece that sent it, the natural language representation of what the agent wanted to communicate, and a move response. The mapping between each field and conversation actions are described in table \ref{tbl:conversationmessagemeaning}. The alternative proposition field starts a new move discussion if present and the same logic is applied as though it was the the move response in the original conversation message.
...
\begin{table} 
\centering
\caption{Conversation message field meanings.} 
\label{tbl:conversationmessagemeaning}
\begin{tabular}{ l|l|l } 
	Field & Predicate & Meaning \\
 \hline
	response & == null & Request Proposals \\ 
	response.opinion & != null & Voice Opinion \\ 
	response.reasoning & != null & * with Justification \\ 
	response.alternativeProposition & != null & Propose Alternative \\ 
	response.performed & == true & Perform Move \\ 
 \hline
\end{tabular}
\end{table}
